...
export function duplicateModelIds(
  groups: DraftProviderGroup[],
  officialModelIds: ReadonlySet<string>,
): Set<string> {
  const counts = new Map<string, number>();
  for (const entry of flattenDraftGroups(groups)) {
    const modelId = entry.model.model.trim();
    if (!modelId) continue;
  ...
  return new Set(
    [...counts.entries()]
      .filter(([modelId, count]) => count > 1 || officialModelIds.has(modelId))
      .map(([modelId]) => modelId),
  );
}

export function normalizeDraftDefaults(
  groups: DraftProviderGroup[],
  officialModelIds: ReadonlySet<string>,
): DraftProviderGroup[] {
  const duplicateIds = duplicateModelIds(groups, officialModelIds);
  const defaultKeys = new Map<string, string>();

  for (const entry of flattenDraftGroups(groups)) {
    const modelId = entry.model.model.trim();
    if (
      !duplicateIds.has(modelId) ||
      entry.model.enabled === false ||
      entry.model.isNativeDefault !== true
    ) {
      continue;
    }
    if (!defaultKeys.has(modelId)) {
      defaultKeys.set(modelId, entry.key);
    }
  }
  ...
  return groups.map((group) => ({
    ...
    entries: group.entries.map((entry) => {
      ...
      return {
        ...
        model: {
          ...model,
          ...(defaultKeys.get(modelId) === entry.key
...
export function setDraftDefault(
  groups: DraftProviderGroup[],
  modelId: string,
  entryKey: string,
): DraftProviderGroup[] {
  return groups.map((group) => ({
    ...group,
    entries: group.entries.map((entry) => {
      if (entry.model.model.trim() !== modelId) return entry;
      const { isNativeDefault: _isNativeDefault, ...model } = entry.model;
      return {
        ...entry,
        model: {
          ...model,
          ...(entry.key === entryKey ? { isNativeDefault: true } : {}),
        },
      };
    }),
  }));
