...
- 2026-08-19 已按用户授权将复选框修复版覆盖到标准 Release: `D:\文件\Agenc Cli\cc-switch\src-tauri\target\release\cc-switch.exe`, 版本 `3.20.0`, PE32+ x64, 大小 `34,208,768` bytes, SHA256 `CB80FE9C9B843F6A080D7F3AE4158CFA1ABFF8203134DFD892620550BAC681DF`. 旧下载目录实例已停止, 当前正式实例 PID `22736` 从标准路径运行;正式数据库、设置和登录状态未改动.`r`n- 2026-08-20 本轮智能排序修复已通过定向 Vitest 13/13、`pnpm typecheck`、`pnpm format:check`、`pnpm build:renderer`、`git diff --check` 和正式配置 Windows x64 Release 构建. 全量 Vitest 首次并发运行 142 个文件/1034 个测试时有 2 个 PiProviderForm 超时, 单独重跑该文件 46/46 通过. 隔离 GUI 实际打开 Codex 模型菜单,确认 `智能排序`、`智能排序规则` 入口和按显示名排序的模型列表;未点击保存,未修改正式数据库. 标准 Release 已覆盖到 `D:\文件\Agenc Cli\cc-switch\src-tauri\target\release\cc-switch.exe`,版本 `3.20.0`,PE32+ x64,大小 `34,208,768` bytes,SHA256 `B0B7E6E46F1C91172ED105AB73616635BC71466839A11C74BE9384447E916CCE`. 当前实际运行仍是非标准 Downloads 实例 PID `22908`,路径 `C:\Users\jery3\Downloads\cc-switch-v3.20.0-custom.1\CC-Switch-v3.20.0-custom.1-Windows-x64.exe`;未强制结束该实例.

## 未完成边界与回归重点

- 为保护正在进行的 Codex 对话, 未对真实 `ChatGPT.exe` 执行破坏性重启, 未在真实 Desktop 会话中验证第三方模型菜单读取和对话级路由.
- 标准路径中的 `v3.20.0` Release 已更新为 SHA256 `B0B7E6E46F1C91172ED105AB73616635BC71466839A11C74BE9384447E916CCE`; 重新启动时因非标准 Downloads 实例 PID `22908` 持有相同单实例标识,未接管当前运行状态. 本轮未在真实 Codex Desktop 会话中验证第三方模型菜单读取和对话级路由,未用真实供应商凭据现场执行 `/models` 获取. `ccswitch` 协议注册未在本轮改动.
