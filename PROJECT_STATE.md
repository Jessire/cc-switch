...
- 2026-08-26 修复 adaptive 思考模型在工具续轮丢失推理强度的缺陷: `src-tauri/src/proxy/providers/transform_codex_anthropic.rs` 将 adaptive 分支提到 `thinking_history_is_valid` 之前, 并让强制 tool_choice 的降级只作用于 legacy budget 路径, 使 `claude-opus-5` 等 adaptive 模型在无签名 thinking block 的工具续轮上仍发送 `thinking:{"type":"adaptive"}` 与顶层 `output_config.effort`. 根因: 该中转从不返回 thinking block, 导致每次工具续轮的 `trailing_turn_supports_thinking` 必然失败并被预防性降级为 `thinking:{"type":"disabled"}`, 上游探针实测该形状返回 HTTP 200, 降级无必要. 验证: `transform_codex_anthropic` 模块单测 81/81, 更广 proxy 回归 1424/1424, `cargo fmt` 已应用; 旧二进制真实出站基线证明 plain 六档全部带 effort 而 tool_continuation 六档全部 `disabled`+`output_config=null`; 标准 Windows x64 Release 已重新构建 (`cargo build --release`, EXIT=0). 未完成: 新二进制的真实出站报文验证需要重启 cc-switch, 当前 PID 13756 (启动于 13:30:31, 旧构建) 正承载用户会话, 未强制结束.
