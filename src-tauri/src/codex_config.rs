...
/// Legacy priority boundary used by the reverse parser to ignore official rows
/// from catalogs generated by earlier builds that re-attached Codex's bundled
/// models. Current catalogs contain third-party provider rows only.
...
fn codex_model_catalog_from_specs(
    ...
    // The catalog intentionally contains third-party provider rows only. Codex's
    // `model_catalog_json` replaces the bundled picker, which is exactly what the
    // customized Desktop menu needs: official entries must never leak back in.
...
fn codex_model_catalog_from_settings(
    ...
    if specs.is_empty() {
        // An empty managed catalog is intentional: keeping the pointer to
        // `{ "models": [] }` prevents Codex Desktop from falling back to its
        // bundled official model menu when no third-party model is selected.
...
/// Generate Codex `model_catalog_json` from provider settings and keep the
/// top-level TOML field pointing to the generated third-party-only catalog.
...
/// correctly; an empty inline catalog still projects to a managed empty catalog
/// so Codex Desktop cannot fall back to official models, while an absent key leaves
/// the text untouched. This is
...
"#;
    ...
    #[test]
    fn empty_model_catalog_stays_managed_and_hides_bundled_models() {
        let catalog = codex_model_catalog_from_settings(
            &json!({ "modelCatalog": { "models": [] } }),
            "model = \"gpt-5.6-sol\"\n",
            CodexCatalogToolProfile::NativeResponses,
        )
        .expect("empty catalog generation should not error")
        .expect("empty catalog should remain managed");
    ...
    fn third_party_gpt_catalog_exposes_reasoning_levels_and_fast_tier() {
        ...
        let specs = vec![CodexCatalogModelSpec {
            ...
            display_name: Some("GPT 5.6 Relay".to_string()),
    ...
    fn codex_routed_catalog_applies_each_rows_tool_profile() {
        ...
        let specs = vec![
            CodexCatalogModelSpec {
                ...
                display_name: Some("Chat Relay - GPT 5.4".to_string()),
                context_window: Some(128_000),
                supports_parallel_tool_calls: None,
                input_modalities: None,
                base_instructions: None,
            ...
            CodexCatalogModelSpec {
                ...
                display_name: Some("Native Relay - GPT 5.4".to_string()),
                context_window: Some(128_000),
                supports_parallel_tool_calls: None,
                input_modalities: None,
                base_instructions: None,
            ...
            CodexCatalogModelSpec {
                ...
                display_name: Some("Anthropic Relay - Claude Sonnet 4".to_string()),
                context_window: Some(128_000),
                supports_parallel_tool_calls: None,
                input_modalities: None,
                base_instructions: None,
...
"#;
    ...
    #[test]
    fn codex_model_catalog_excludes_all_bundled_models() {
        let mut template = json!({ "slug": "gpt-5.5" });
        fill_template_fields_from_static(&mut template);
        let specs = vec![CodexCatalogModelSpec {
            model: "k3".to_string(),
        ...
        assert_eq!(models.len(), 1, "official bundled rows must be excluded");
        assert_eq!(
    ...
    #[test]
    fn codex_model_catalog_keeps_only_custom_row_for_official_slug() {
        let template = json!({ "slug": "gpt-5.5" });
        let specs = vec![CodexCatalogModelSpec {
            model: "gpt-5.5".to_string(),
            display_name: Some("Relay GPT 5.5".to_string()),
            context_window: Some(128_000),
            supports_parallel_tool_calls: None,
            input_modalities: None,
            base_instructions: None,
        ...
        assert_eq!(
            ...
            Some("Relay GPT 5.5"),
            "only the third-party provider row should remain"
        );
    }
